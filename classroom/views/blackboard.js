  ...
  if (user.role === 'teacher' && recentLesson.options.includes('chalkboard')) {
    return `
      <div class="mt-3 d-flex justify-content-end">
        ...
        <button class="btn btn-sm btn-primary" onclick="cleanChalkboard('${recentLesson.group}');">${locale.buttons.clean_chalkboard[config.lang]}</button>
    ...
    <div class="mt-3 d-flex justify-content-between">
      <div class="font-weight-bold">
        ${locale.headlines.navi_teacher[config.lang]}: ${getTitleNameById(recentLesson.teacher.id)}
      </div>
      <div>
        <button class="btn btn-sm btn-danger mr-3" onclick="exitClassroom();">${locale.buttons.exit_onlinelesson[config.lang]}</button>
        <button class="btn btn-sm btn-warning" onclick="signalTeacher('${recentLesson.group}','${user.id}')">
