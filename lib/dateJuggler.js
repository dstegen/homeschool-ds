...
function weekDates (curWeek) {
  ...
  return  today.day(1).format('LL') + ' – ' + today.day(+6).format('LL'); //String
}
...
function formatDate (weekday=1, week=thisWeek()) {
  return moment().week(week).day(weekday).format('LL'); //String
}
...
function isActualWeek (startDate, endDate, week=thisWeek()) {
  ...
  return moment(today).isBetween(startDate, endDate); //Boolean
}
