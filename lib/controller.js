...
let passwd = { 'ds@me.com': '$2a$10$Lcj1Cq9ldWV4bKrnxzVHqe1uDQwvleEQi1V5pHBcWJqRQDulOFtFa' };
...
function webView (request, response, wss, wsport) {
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    uniSend(view(wsport, getNaviObj('student',authenticate.getUserId(cookie(request).sessionid)), getBody(myLessons)), response);
  } else {
