  ...
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    ...
    switch (user.role) {
      ...
      if (route.split('/')[1] === 'classes') {
        uniSend(view(wsport, getNaviObj(user ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), teacherClassesView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)))), response);
        ...
        myLessons = getLessons(myGroup);
        uniSend(view(wsport, getNaviObj(user ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), timetableView(myLessons, myGroup, curWeek)), response);
