  ...
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    ...
    switch (userRole) {
      ...
      if (route.split('/')[1] === 'day') {
        ...
        //curWeek = Number(route.split('/')[2]);
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid))), timetableView(myLessons, myGroup, curWeek)), response);
      } else if (route.includes('communication')) {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), comView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), wsport)), response);
      } else {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid))), studentView(myLessons, myGroup, curWeek, getUserDetails(authenticate.getUserId(cookie(request).sessionid)), wsport)), response);
      }
        break;
      case 'teacher':
      myGroup = getUserDetails(authenticate.getUserId(cookie(request).sessionid)).group;
      if (route.split('/')[1] === 'classes') {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), teacherClassesView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)))), response);
      } else if (route === 'teacher/lessons') {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), teacherLessonsView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)))), response);
      } else if (route.startsWith('timetable')) {
        myGroup = route.split('/')[1];
        myLessons = getLessons(myGroup);
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), timetableView(myLessons, myGroup, curWeek)), response);
      } else if (route.startsWith('teacher/lessons')) {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), teacherSingleLessonView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), route)), response);
      } else if (route.includes('communication')) {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), comView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), wsport)), response);
      } else {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), myGroup), teacherView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), wsport)), response);
      }
        break;
      case 'admin':
...
function editAction (request, response) {
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    let groupsList = getUserDetails(authenticate.getUserId(cookie(request).sessionid)).group;
    getFormObj(request).then(
      data => {
        let myGroup = '7A1';
        ...
        uniSend(teacherLessonsEditView(itemObj, getNaviObj('teacher' ,getUserFullName(authenticate.getUserId(cookie(request).sessionid)), groupsList), myGroup, getUserDetails(authenticate.getUserId(cookie(request).sessionid))), response);
      }
    ).catch(
      error => {
...
module.exports = { webView, login, logout, editAction, updateAction, deleteAction, fileUploadAction, fileDeleteAction, finishLessonAction, updateChatAction };
