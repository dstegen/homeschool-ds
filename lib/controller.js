  ...
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    ...
    switch (userRole) {
      ...
      if (route.split('/')[1] === 'classes') {
        ...
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid))), teacherSingleLessonView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), route)), response);
      } else {
        uniSend(view(wsport, getNaviObj(userRole ,getUserFullName(authenticate.getUserId(cookie(request).sessionid))), teacherView(getUserDetails(authenticate.getUserId(cookie(request).sessionid)), wsport)), response);
      }
        break;
      case 'admin':
