...
const teacherLessonsEditView = require('../teacher/views/edit-lesson-view');
  ...
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    ...
    switch (user.role) {
      ...
      if (route.split('/')[1] === 'day') {
        uniSend(view(wsport, getNaviObj(user ,getUserFullName(authenticate.getUserId(cookie(request).sessionid))), studentDayView(myLessons, myGroup, curDay, getUserDetails(authenticate.getUserId(cookie(request).sessionid)))), response);
