  ...
  if (authenticate.loggedIn(cookie(request).sessionid) && getUserDetails(authenticate.getUserId(cookie(request).sessionid)).role === 'admin') {
    getFormObj(request).then(
      data => {
        ...
        uniSend(view('', getNaviObj(getUserDetails(authenticate.getUserId(cookie(request).sessionid))), settingsView()),response);
      }
    ).catch(
      error => {
...
module.exports = { webView, login, logout, editAction, updateAction, deleteAction, fileUploadAction, fileDeleteAction, finishLessonAction, updateChatAction, updatePrivateMessagesAction, updateUserAction, setPasswordAction, updatePasswordAction, editUserAction, settingsAction };
