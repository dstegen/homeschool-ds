  ...
  if (authenticate.loggedIn(cookie(request).sessionid)) {
    ...
    switch (user.role) {
      ...
      case 'teacher':
        ...
        break;
      case 'admin':
        uniSend(view(wsport, naviObj, adminView()), response);
        break;
    }
  } else {
...
function updatePasswordAction (request, response) {
  if (authenticate.loggedIn(cookie(request).sessionid) && getUserDetails(authenticate.getUserId(cookie(request).sessionid)).role !== 'admin') {
    getFormObj(request).then(
      data => {
        if (bcrypt.compareSync(data.fields.password, passwd[data.fields.userId])) {
          ...
          console.log('- ERROR old password didn\'t match for userId: '+data.fields.userId);
...
function editUserAction (request, response) {
  if (authenticate.loggedIn(cookie(request).sessionid) && getUserDetails(authenticate.getUserId(cookie(request).sessionid)).role === 'admin') {
    if (request.url.split('/')[3] != undefined && Number.isInteger(Number(request.url.split('/')[3]))) {
      uniSend(view('', getNaviObj(getUserDetails(authenticate.getUserId(cookie(request).sessionid))), editUserView(getUserById(Number(request.url.split('/')[3])))), response);
    } else {
      uniSend(view('', getNaviObj(getUserDetails(authenticate.getUserId(cookie(request).sessionid))), editUserView()), response);
    }
  } else {
    uniSend(loginView(), response);
  }
...
module.exports = { webView, login, logout, editAction, updateAction, deleteAction, fileUploadAction, fileDeleteAction, finishLessonAction, updateChatAction, updatePrivateMessagesAction, updateUserAction, setPasswordAction, updatePasswordAction, editUserAction };
