...
initUsers();
const { getLessons, updateLesson, deleteLesson, finishLesson, deleteFileFromLesson, deleteFileFromLessonFinished } = require('../models/model-lessons');
...
const { updateTopic, updateCard, deleteFromBoard, deleteFileFromCard } = require('../models/model-board');
        ...
        if (user.role === 'student') {
          ...
          if (fileUpload(data.fields, data.files, filePath)) {
            let addFields = {
              ...
              studentId: user.id,
              file: path.join(filePath, data.files.filetoupload.name)
            }
      ...
      data => {
        ...
        if (fileDelete(data.fields)) {
          if (data.fields.section === 'cards') {
            ...
            deleteFileFromLessonFinished(data.fields.group, Number(data.fields.lessonId), Number(data.fields.studentId), '/data/classes/'+data.fields.filePath+data.fields.delfilename);
          } else {
            deleteFileFromLesson(data.fields.group, Number(data.fields.courseId), '/data/classes/'+data.fields.filePath+'/'+data.fields.delfilename);
          }
        }
        uniSend(new SendObj(302, [], '', urlPath), response);
      }
