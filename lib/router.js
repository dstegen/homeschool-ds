...
const { webView, login, logout, editAction, updateAction, deleteAction, fileUploadAction, fileDeleteAction, finishLessonAction, updateChatAction, updatePrivateMessagesAction } = require('./controller');
...
function router (request, response, wss, wsport) {
  ...
  if (request.url.includes('data') || request.url.includes('node_modules') || request.url.includes('public') || request.url.includes('favicon')) {
   ...
   updateChatAction(request, response, wss, wsport);
 } else if (request.url.startsWith('/message')) {
   updatePrivateMessagesAction(request, response, wss, wsport);
 } else {
